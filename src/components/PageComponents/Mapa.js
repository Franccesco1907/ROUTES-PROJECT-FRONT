...
import PlanTransporteGeneral from "../../pages/Administrador/Simulaciones/PlanTransporteGeneral";
...
function Mapa(props) {
  ...
  //const currClockRef = useRef(iniClock);
  ...
  const isAnimRunRef = useRef(false);
  ...
  UtilsFunction.useEffectOnce(() => {
    truckPlanService
      .initSimulation("7days", dateToStringParam(clockRef.current))
      .then((id) => {
        idSimRef.current = id;
  ...
  const llamarData = () => {
    if (clockRef.current >= endClock) {
      //alert("fin simulacion");
      truckPlanRef.current.push(undefined);
      return;
    }
    currClock = dateToStringParam(clockRef.current);
    fechaTypeDate = datePlusMin(clockRef.current, scFactor);
    if (fechaTypeDate >= endClock) fechaTypeDate = endClock;
    nextClock = dateToStringParam(fechaTypeDate);
    console.log(`Algoritmo run ${runRef.current} inicia`);
    truckPlanService
      .runAlgorithm(idSimRef.current, runRef.current, currClock, nextClock)
    ...
    <>
      <div className="mapContainer">
        ...
        >
          ...
          {props.begin ? ( // && Math.floor(props.timer / 86400) < 7
            <TruckPlanController
      ...
      {idSimRef.current != undefined  && Array.isArray(params.truckPlans)? (
        <div>
          <PlanTransporteGeneral
            idSimRef={idSimRef}
            truckPlanRef={params.truckPlans}
          />
        </div>
      ) : (
        <></>
