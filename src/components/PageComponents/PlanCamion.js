...
const distFromStart = (travel, intPoint, currDate, dictVertex) => {
  ...
  let vX = endVertex.x - startVertex.x;
  let vY = endVertex.y - startVertex.y;
  let magVector = Math.sqrt(Math.pow(vX, 2) + Math.pow(vY, 2));
  let uFactorX = vX / magVector;
  let uFactorY = vY / magVector;
  //porcentaje de distancia recorrido desde el inicio
  let time =
    (currDate.getTime() - dateDeparture.getTime()) / (travel.edgeTime * 3.6e6);

  intPoint.x = startVertex.x + dist * time * uFactorX;
  intPoint.y = startVertex.y + dist * time * uFactorY;
};
...
const crearPath = (travels, dictVertex, currDate) => {
  ...
  for (let i = 0; i < travels.length; i++) {
    ...
    //Si aun no sale del startVertex
      ...
      distFromStart(travels[i], intPoint, lastArrival, dictVertex);
...
export default function PlanCamion(props) {
  ...
  const addInput = (id) => {
    props.setNotify({
      isOpen: true,
      message: `Soy un camion con id ${id}`,
      type: "info",
    });
  };
    ...
    <>
      ...
      >
        <animateMotion
          ...
          className="animation"
          key={`anim${props.keys}`}
          dur={`${(props.tot.time * 60 * 60) / Math.pow(2, props.accell - 1)}`}
